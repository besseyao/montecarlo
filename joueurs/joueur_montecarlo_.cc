...
void coupRandom (Jeu jeu,Brix &coup){
...
void simulePartie (Jeu &jeu,arbreNaire n){
    Brix coup;
    while (!jeu.fini()){
        coupRandom(jeu,coup);
        jeu.joue(coup);
...
void monteCarlo (arbreNaire &n,Jeu &jeu) {
    arbreNaire n;
    n.constructionRacine;
    int limiteNbSimulation=1;
